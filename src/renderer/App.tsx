  ...
  return <div className="dashboard-shell">
    ...
    <main className="dashboard-main">
      ...
      <section className="dashboard-content">
        ...
        {props.tab === 'library' && (props.files.length ? <div className="file-list">{props.files.map((file) => <div key={file.id}><div><strong>{file.filenameLocal}</strong><span>{file.localPath ?? 'Nur in Notion gespeichert'}</span></div><small>{formatBytes(file.sizeBytes)} · {file.status}</small></div>)}</div> : <EmptyState title="Noch keine Dateien" text="Starte den ersten Sync, um die lokale Bibliothek zu füllen." />)}
