...
* **MCP-Sicherheit**: Die optionale MCP-Schnittstelle muss explizit freigegeben werden. stdio und der Bearer-geschützte Loopback-SSE-Endpunkt sind lokal; es gibt keine automatische Cloud-Exponierung. Der kontoweite read-only Zugriff ist nicht auf synchronisierte Kurse beschränkt und wird im UI deutlich erklärt.
